...
# ================= 测试场景5：地址计算前递 =================
# 测试链：ADDI->SW->ADDI->LW（地址生成前递）
addi x15, x0, 0x10        # x15= 0x10
sw   x15, 16(x1)          # [0x110] = 0x10（使用x15作为地址偏移）
addi x15, x15, 0x10       # x15= 0x20（修改基址寄存器）
lw   x16, -16(x15)        # x16= [0x20-16]=[0x10]=0x10（需前递修改后的x15）
